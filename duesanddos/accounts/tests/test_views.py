...
class AuthAndProfileTests(TestCase):
    ...
    def test_login_deactivated_user(self):
        ...
        self.assertEqual(
            self.client.session.get("pending_reactivation_user_id"), self.user.id
        )
    ...
    def test_deactivate_account_view_post_wrong_password(self):
        response = self.client.post(
            reverse("deactivate_account"), {"confirmation": "wrong"}
        )
        self.assertRedirects(response, reverse("profile"))

    def test_deactivate_account_view_post_correct_password(self):
        response = self.client.post(
            reverse("deactivate_account"), {"confirmation": TEST_PASSWORD}
        )
        self.assertRedirects(response, reverse("login"))
        self.user.refresh_from_db()
    ...
    def test_deactivate_account_view_post_no_password_correct_word(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(
            reverse("deactivate_account"), {"confirmation": "DEACTIVATE"}
        )
        self.assertRedirects(response, reverse("login"))
        self.user.refresh_from_db()
        self.assertTrue(self.user.is_deactivated)
