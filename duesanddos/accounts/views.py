...
def profile_view(request):
        ...
        if "save_profile" in request.POST:
            ...
            if user_form.is_valid() and profile_form.is_valid():
                ...
                for field, errors in user_form.errors.items():
                    label = user_form.fields[field].label if field in user_form.fields else field.replace("_", " ").title()
                    for error in errors:
                        messages.error(request, f"{label}: {error}")
                for field, errors in profile_form.errors.items():
                    label = profile_form.fields[field].label if field in profile_form.fields else field.replace("_", " ").title()
                    for error in errors:
                        messages.error(request, f"{label}: {error}")
                return redirect("profile")
            ...
            # Fallback: POST received but no recognized action key — initialize forms unbound
            user_form = UserUpdateForm(instance=request.user)
            profile_form = ProfileUpdateForm(instance=profile)
            password_form = CustomPasswordChangeForm(request.user)
    else:
    ...
    memberships = HouseholdMember.objects.filter(user=request.user).select_related('household')
...
def household_settings_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "switch_household":
            ...
            if household_id:
                ...
                member_link = HouseholdMember.objects.filter(user=request.user, household_id=household_id).first()
                if member_link:
        ...
        # 2. Create Household
            ...
            if name:
                ...
                HouseholdMember.objects.create(user=request.user, household=new_household, role="Admin")
        ...
        # 3. Join Household
            ...
            if code:
                ...
                if household:
                    if household.invite_code_expires and household.invite_code_expires > timezone.now():
                        member, created = HouseholdMember.objects.get_or_create(user=request.user, household=household, defaults={'role': 'Member'})
                        if created:
                            profile.active_household = household
                            profile.save()
                            messages.success(request, f"Successfully joined {household.name}!")
                        else:
        ...
        is_admin = HouseholdMember.objects.filter(user=request.user, household=active_household, role="Admin").exists()
        ...
        # 5. Generate Invite Code
            ...
            if is_admin:
                ...
                active_household.invite_code_expires = timezone.now() + timedelta(days=7)
                active_household.save()
        ...
        # 6. Update Role
            ...
            if is_admin:
                ...
                if new_role in dict(HouseholdMember.ROLE_CHOICES).keys():
                    member_link = HouseholdMember.objects.filter(user_id=target_user_id, household=active_household).first()
                    if member_link:
                        # Prevent last admin from demoting themselves
                        if member_link.user == request.user and new_role != "Admin":
                            admin_count = HouseholdMember.objects.filter(household=active_household, role="Admin").count()
                            if admin_count <= 1:
                                messages.error(request, "You cannot demote yourself as the only admin.")
                                return redirect("household_settings")
                ...
                if member_link:
                    ...
                    if member_link.user == request.user and member_link.role == "Admin":
                        ...
                        if admin_count <= 1:
                            ...
                            if new_admin_id:
                                new_admin_link = HouseholdMember.objects.filter(user_id=new_admin_id, household=active_household).first()
                                if new_admin_link:
                                    new_admin_link.role = "Admin"
                                    new_admin_link.save()
                                else:
                                    messages.error(request, "Selected successor not found.")
                                    return redirect("household_settings")
                            else:
                                messages.error(request, "As the only admin, you must either assign another admin or delete the household instead.")
                                return redirect("household_settings")
                    ...
                    if str(request.user.id) == str(target_user_id):
                        ...
                        next_membership = HouseholdMember.objects.filter(user=request.user).first()
                        profile.active_household = next_membership.household if next_membership else None
                        profile.save()
    ...
    my_memberships = HouseholdMember.objects.filter(user=request.user).select_related('household')
    ...
    if active_household:
        members = HouseholdMember.objects.filter(household=active_household).select_related('user')
        is_admin = HouseholdMember.objects.filter(user=request.user, household=active_household, role="Admin").exists()
        
        if active_household.invite_code and active_household.invite_code_expires and active_household.invite_code_expires > timezone.now():
