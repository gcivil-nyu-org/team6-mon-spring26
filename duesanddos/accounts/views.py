...
def dashboard_view(request):
    ...
    # Sum up all expenses for the current active household
    total_spent = 0
    if active_household:
...
def expense_history_view(request):
    ...
    if not active_household:
        messages.warning(request, "Please select or create a household to view expenses.")
        return redirect("household_settings")

    # Fetch expenses for the active household, newest first
    expenses = Expense.objects.filter(household=active_household).select_related("payer")
